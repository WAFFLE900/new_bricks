...
def auth_error(status):
    ...
    if status == 401:
        response_object['message'] = "Invalid Token or Token Expired"
...
def tag_index():
    ...
    try:
        ...
        result = (
            ...
            .filter(Project.id == id, Project.user_id == post_data.get('user_id'))
...
def add_tag():
    ...
    try:
        ...
        project_id = (
            ...
            .filter(User.id == post_data.get('user_id'))
            .scalar()
        )
...
def add_record():
    ...
    if not project_exists:
        ...
        response_object['message'] = '專案不存在'
        return jsonify(response_object), 400
    record_exists = session.query(exists().where(Record.record_name == record_name)).scalar()
    if record_exists:
        response_object['status'] = 'failed'
...
def get_record_index():
    ...
    try:
        ...
        record_get = (
            ...
            .filter(Project.user_id==post_data.get("user_id"))
...
def edit_record():
    ...
    record_exists = session.query(exists().where(
            ...
            Record.user_id==post_data.get("user_id"))).scalar()
...
def delete_record():
    ...
    try:
        ...
        record_count = session.query(Record).filter(Record.id == post_data.get("record_id"),Record.user_id==post_data.get("user_id")).count()
        if record_count == 0:
            response_object["status"] = "failed"
...
def add_textBox():
    ...
    record_exists = session.query(exists().where(Record.id == record_id),Record.user_id==post_data.get("user_id")).scalar()
    if not record_exists:
        response_object['status'] = 'failed'
        response_object['message'] = '會議記錄不存在'
        return jsonify(response_object), 400
...
def recover_record():
    ...
    try:
        ...
        if record_count == 0:
            ...
            response_object["message"] = "查無紀錄"
            return jsonify(response_object)
        session.query(Record).filter(Record.id == post_data.get("record_id"),Record.user_id==post_data.get("user_id")).update({"record_trashcan":0})
        session.commit()
...
def delete_record_permanent():
    ...
    try:
        ...
        session.query(Record).filter(Record.id == post_data.get("record_id"),Record.user_id==post_data.get("user_id")).delete()
        session.commit()
        response_object["message"] = "刪除成功"
